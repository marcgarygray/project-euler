...
class LargestProduct {
    constructor(length) {
        this.length = length;
        this.hugeNumberAsString = '';
    }
    get product() {
        ...
        while (remainingString.length >= this.length) {
            ...
            const currentProduct = currentGroup.reduce((product, current) => {return parseInt(product) * parseInt(current)});
            if (currentProduct > largestProduct) largestProduct = currentProduct;
            remainingString = remainingString.substring(0, remainingString.length - 1);
        }
